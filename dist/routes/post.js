...
postRoutes.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const posts = yield post_model_1.Post.find()
        .sort({ _id: -1 }) // De forma descendente
        .skip(skip) // Salteo registros
        .limit(10) // de 10 en 10
        .populate('usuario', '-password') // muestra la informacion del usuario
        .exec();
    res.json({
        ok: true,
        pagina,
...
postRoutes.post('/', [autentication_1.verificaToken], (req, res) => {
    ...
    const imagenes = fileSystem.imagenesDeTempHaciaPost(req.usuario._id); // leo del token el usuario id
    body.imgs = imagenes;
...
postRoutes.post('/upload', [autentication_1.verificaToken], (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield fileSystem.guardarImagenTemporal(file, req.usuario._id).catch((error) => { console.log(error); });
    res.json({
        ok: true,
        file: file.mimetype
    });
